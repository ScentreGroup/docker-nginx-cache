...
ENV NGINX_CACHE_PURGE_VERSION 2.4.2
...
# START
 ...
 && curl -fSL https://github.com/nginx-modules/ngx_cache_purge/archive/$NGINX_CACHE_PURGE_VERSION.tar.gz -o ngx_cache_purge.tar.gz \
 # END
 ...
 ; do \
  ...
  gpg --keyserver "$server" --keyserver-options http-proxy="$http_proxy",timeout=10 --recv-keys "$GPG_KEYS" && found=yes && break; \
 done; \
